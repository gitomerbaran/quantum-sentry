...
async fn main() {
    ...
    let nats_depth_handle =
        tokio::spawn(nats_ingest_depth_loop(nats_url.clone(), tx_depth));
...
async fn nats_ingest_depth_loop(
    nats_url: String,
    tx: mpsc::Sender<DepthSnapshotRow>,
) {
    let mut backoff_ms: u64 = 250;
    let backoff_max_ms: u64 = 5_000;
...
async fn nats_ingest_liq_loop(
    nats_url: String,
    tx: mpsc::Sender<LiquidationTickRow>,
) {
    let mut backoff_ms: u64 = 250;
    let backoff_max_ms: u64 = 5_000;
